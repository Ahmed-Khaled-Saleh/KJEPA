...
def load_pretrained(
    ...
    if load_predictor:
        # -- loading predictor
        pretrained_dict = checkpoint["knet"]
        pretrained_dict = {k.replace("backbone.", ""): v for k, v in pretrained_dict.items()}
        msg = knet.load_state_dict(pretrained_dict, strict=False)
        logger.info(f"loaded pretrained knet from epoch {epoch} with msg: {msg}")

# -- loading target_encoder
    print(list(checkpoint.keys()))
    pretrained_dict = checkpoint[target_encoder_key]
    pretrained_dict = {k.replace("backbone.", ""): v for k, v in pretrained_dict.items()}
    msg = ae.load_state_dict(pretrained_dict, strict=False)
...
class AutoEncoder(nn.Module):
    """
    Simple Autoencoder for latetns
    """

    def __init__(self, traj_length= 16, dim=1408, hidden_dim=1024, bottleneck_dim=512):
        super().__init__()
    ...
    def forward(self, z):
        """
        z: tensor of shape (B, 16, 1408)
        """
        # Encode each patch -> (B, traj_length, bottleneck_dim)
        y = self.encoder(z)

        # Decode -> (B, traj_length, traj_length)
        z_rec = self.decoder(y)

        # Reshape back -> (B, H, W, D)
        # x_rec = x_rec.view(B, H, W, D)
...
def init_video_model(
    ...
    ae = AutoEncoder(traj_length= 16, dim=1408, hidden_dim=1024, bottleneck_dim=512)
    knet = Knet(size= 512)
    encoder.to(device)
    ae.to(device)
    knet.to(device)
...
def init_opt(
    ...
    param_groups = [
        ...
        {
            "params": (p for n, p in ae.named_parameters() if ("bias" not in n) and (len(p.shape) != 1)),
        },
        {
            "params": (p for n, p in knet.named_parameters() if ("bias" not in n) and (len(p.shape) != 1)),
        },
        {
        ...
        {
            "params": (p for n, p in ae.named_parameters() if ("bias" in n) or (len(p.shape) == 1)),
            "WD_exclude": zero_init_bias_wd,
            "weight_decay": 0,
        },
        {
            "params": (p for n, p in knet.named_parameters() if ("bias" in n) or (len(p.shape) == 1)),
            "WD_exclude": zero_init_bias_wd,
            "weight_decay": 0,
        },
    ]
