...
from app.vjepa_koopman.droid import init_data
from app.vjepa_koopman.transforms import make_transforms
from app.vjepa_koopman.utils import init_opt, init_video_model, load_checkpoint, load_pretrained
...
def main(args, resume_preempt=False):
    ...
    # -- init model
    encoder, ae, knet = init_video_model(
    ...
    if compile_model:
        logger.info("Compiling encoder, target_encoder, and knet.")
    ...
    knet = DistributedDataParallel(knet, static_graph=False, find_unused_parameters=True)
    ae = DistributedDataParallel(ae, static_graph=False, find_unused_parameters=True)
    for p in encoder.parameters():
        p.requires_grad = False

    # -- load pretrained weights
    encoder, ae, knet = load_pretrained(
        r_path=p_file,
        encoder=encoder,
        ae= ae,
        knet=knet,
        context_encoder_key=context_encoder_key,
        target_encoder_key="autoencoder",
            ...
            def train_step():
                ...
                def forward_target(c):
                    ...
                    with torch.no_grad():
                        ...
                        h = encoder(c)
                        h = h.view(2, max_num_frames, -1, h.size(-1)).flatten(1, 2)
                        if normalize_reps:
                            h = F.layer_norm(h, (h.size(-1),))
                            
                        h = h.view(2, 16, 256, 1408)
                        z_t = h.mean(dim=2)
                        return z_t
                ...
                #     return z_tf, z_ar
                def evolve_from_z0(Knet, z0, steps=16):
                    """
                    z0: [B, D] initial state
                    steps: int, rollout length
                    """
                    preds = torch.zeros(z0.size(0), steps, z0.size(1), device=z0.device)
                    preds[:, 0] = z0
                    for t in range(1, steps):
                        preds[:, t] = Knet(preds[:, t-1].clone())
                    return preds  # [B, steps, D]
                ...
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    z = forward_target(clips)
                    z_rec, y = ae(z)  # z_rec = [B, T, D], y = [B, T, D_y]
                    y_hat = evolve_from_z0(knet, y[:,0,:])  # y_hat = [B, T, D_y]
                    z_hat = ae.decoder(y_hat)  # z_hat = [B, T, D]

                    loss_rec = loss_fn(z_rec, z)
                    loss_linearity = loss_fn(y_hat[:, 1:, :], y[:, 1:, :])
                    loss_predict = loss_fn(z_hat[:, 1:, :], z[:, 1:, :])
                    loss = loss_rec + loss_linearity + loss_predict
                    loss = jloss + sloss
